-- Restore pre-scoped behavior: global scan of all RANGE partitions (any parent).
...
DECLARE
    ...
    partitionTable TEXT;
BEGIN
    -- Below query select all the tables which were created for the date range before
    -- $tableDate and capture names of all such tables in $partitionTables[]
    ...
    WHERE nn.min_rangeval < tableDate::date;
