...
> **Node right-sizing and feature enablement:** 2026-05-10 — Multiple changes across ros-ocp-backend, koku-metrics-operator, and nise. (1) **Node recommendations enabled by default**: Removed `ROS_ENABLE_NODE_RECS` feature gate — node utilization analysis (underutilized, overcommitted, stranded) runs unconditionally. (2) **Namespace recommendations enabled by default**: On-prem always enabled; cloud gated only by Unleash kill switch `rosocp.namespace_disabled` for rollback. (3) **Tier 2 operator capacity data**: koku-metrics-operator now emits `node_capacity_cpu_cores` and `node_capacity_memory_bytes` in ROS CSVs; ros-ocp-backend CSV parser and digest pipeline wire capacity into `max_cpu_allocatable_mc` / `max_mem_allocatable_kib` in `daily_node_digests`; `resolveAllocatable()` prefers actual capacity over request-based estimates. (4) **EMA trend smoothing**: Trend slope computation applies EMA smoothing (configurable `ROS_NODE_EMA_ALPHA`, default 0.3) to daily CPU utilization before linear regression, dampening single-day spikes. (5) **EMA imbalance stranded detection**: Replaced the two fixed thresholds (`StrandedHighThreshold` 0.70 / `StrandedLowThreshold` 0.25) with a single EMA-smoothed normalized imbalance score. Per-day imbalance = `|cpu_p95 - mem_p95| / max(cpu_p95, mem_p95)`, smoothed with EMA, flagged when final value exceeds `ROS_NODE_STRANDED_IMBALANCE_THRESHOLD` (default 0.6). Relative metric — works across low/high utilization without absolute thresholds. Transient spikes dampened by EMA. (6) **Nise test data**: nise OCP ROS generator updated to emit `node_capacity_cpu_cores` and `node_capacity_memory_bytes` columns. (7) **Documentation**: 3-day cold start documented in `docs/known-issues.md` with UI null state requirement. Node right-sizing feature section added to known-issues. Env var table updated with `ROS_NODE_STRANDED_IMBALANCE_THRESHOLD` and `ROS_NODE_EMA_ALPHA`.
...
### REQ-5.4: GPU underutilization detection [HIGH]

**Source:** Analysis §17

**Required behavior:** When both GPU core usage and GPU memory usage are below 10% sustained over the recommendation window:
1. Emit `GPU_UNDERUTILIZED` (code 10) notification.
...
| `ROS_GPU_UNDERUTIL_THRESHOLD` | 0.10 | GPU underutilization threshold |
...
| 7 | ~~GPU underutilization threshold?~~ | **RESOLVED** — **10% default, configurable via `ROS_GPU_UNDERUTIL_THRESHOLD`** (already in NFR-5 env table). Rationale: (1) GPU idle detection is inherently noisy — GPU workloads spike during training/inference batches and idle between them; a 5% threshold would produce excessive false positives for batch workloads. (2) 10% matches the industry convention used by NVIDIA Data Center GPU Manager (DCGM) dashboards for "underutilized" alerts. (3) AWS Compute Optimizer uses 10% for GPU underutilization recommendations. (4) The threshold applies to **sustained** underutilization over the entire recommendation window (not instantaneous), so transient idle periods between batches don't trigger it. (5) Configurable via env var for customers with different workload profiles. | ~~Phase 5~~ |
