...
Tag filtering on list endpoints must support existing ROS clients and Koku/HCCM query conventions. The original dual-syntax decision (incorporates former ADR-0121) established that Koku UI sends `filter[tag:key]=value` while legacy clients use `tag=key:value`. Tag data resolves from DB or HTTP sync ([ADR-0119](0119-tags-source-db-on-prem.md), [ADR-0120](0120-saas-http-push-tag-sync.md)) with JSONB on keys ([ADR-0054](0054-resolved-tags-jsonb-on-keys-table.md)).
