...
type ClusterQuotaRec struct {
	...
	Expl                             ClusterQuotaExplanationFactors
}
...
func computeClusterQuotaRecommendation(
	...
	rec.Expl = ClusterQuotaExplanationFactors{
	...
	for _, r := range recs {
		...
		_, err := pool.Exec(ctx, `
			INSERT INTO cluster_quota_recommendation_sets (
				...
				notification_codes, updated_at,`+clusterQuotaExplSQLColumns+`
			) VALUES (
				$1, $2::uuid, $3, $4,
				$5, $6,
				$7, $8, $9,
			...
			DO UPDATE SET
				...
				updated_at = NOW(),`+clusterQuotaExplUpdateSet,
			append([]any{
				...
				r.NotificationCodes,
			}, appendClusterQuotaExplArgs(nil, r.Expl)...)...,
		)
		if err != nil {
