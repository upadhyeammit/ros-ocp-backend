...
func TestConfig_BusinessHoursEnabled_DefaultTrue(t *testing.T) {
	...
	c := GetConfig()
	require.True(t, c.BusinessHoursEnabled)
	require.True(t, BusinessHoursFeatureEnabled())
}

func TestConfig_BusinessHoursEnabled_EnvParsing(t *testing.T) {
	tests := []struct {
		name     string
		envValue string
		setEnv   bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.setEnv {
				t.Setenv("ROS_BUSINESS_HOURS_ENABLED", tt.envValue)
			} else {
				t.Setenv("ROS_BUSINESS_HOURS_ENABLED", "")
			}
			ResetForTest()
			...
			c := GetConfig()
			require.Equal(t, tt.want, c.BusinessHoursEnabled)
			require.Equal(t, tt.want, BusinessHoursFeatureEnabled())
		})
	}
}
