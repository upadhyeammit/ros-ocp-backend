...
func TestGetNotificationCodes_PluginFilterContainer(t *testing.T) {
	...
	req := httptest.NewRequest(
		http.MethodGet,
		"/api/cost-management/v1/recommendations/openshift/notification-codes?filter[plugin]=container",
		nil,
	)
	rec := httptest.NewRecorder()
	e.ServeHTTP(rec, req)
...
func TestGetNotificationCodes_PluginFilterPVC(t *testing.T) {
	e := echo.New()
	v1 := e.Group("/api/cost-management/v1")
	v1.GET("/recommendations/openshift/notification-codes", GetNotificationCodes)
	...
	req := httptest.NewRequest(
		http.MethodGet,
		"/api/cost-management/v1/recommendations/openshift/notification-codes?filter[plugin]=pvc",
		nil,
	)
	rec := httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	...
	assert.ElementsMatch(t, []int16{20, 25, 29, 30}, codes)
}
