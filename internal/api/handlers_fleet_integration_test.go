...
func TestGetFleetSummary_Integration(t *testing.T) {
	...
	_, err = pool.Exec(ctx, `
		...
		VALUES
			($1, $2, 'ns1', 'w1', 'Deployment', 'c1', 'medium_term', 'cost', false, '{}', 10.5, now()),
			($1, $2, 'ns1', 'w2', 'Deployment', 'c2', 'medium_term', 'cost', false, '{5}', 20.0, now()),
			($1, $2, 'ns2', 'w3', 'Deployment', 'c3', 'medium_term', 'cost', false, '{8}', 50.0, now()),
			($1, $2, 'ns2', 'w4', 'Deployment', 'c4', 'medium_term', 'cost', true, '{}', 5.0, now())
