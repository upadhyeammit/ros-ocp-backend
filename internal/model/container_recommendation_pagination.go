...
func nativeContainerPageOrder(pageAlias, orderHow string) string {
	return fmt.Sprintf(
		"%s.ros_container_page_sort %s, %s.cluster_uuid, %s.namespace, %s.workload, %s.container_name",
		pageAlias, orderHow, pageAlias, pageAlias, pageAlias, pageAlias,
	)
}
...
func nativeContainerDetailOrder(orderHow string) string {
	return fmt.Sprintf(
		"page.ros_container_page_sort %s, page.cluster_uuid, page.namespace, page.workload, page.container_name, rs.term, rs.engine",
		orderHow,
	)
}

// nativeContainerSeekAfter returns a WHERE clause for keyset pagination after the cursor row.
func nativeContainerSeekAfter(sortExpr, orderHow string, sortValue interface{}, clusterUUID, namespace, workload, container string) (string, []interface{}) {
	tie := "(rs.namespace, rs.workload, rs.container_name, rs.cluster_uuid)"
	if orderHow == listoptions.OrderDesc {
		return fmt.Sprintf(
			"((%s) < ? OR ((%s) IS NOT DISTINCT FROM ? AND %s > (?, ?, ?, ?)))",
			sortExpr, sortExpr, tie,
		), []interface{}{sortValue, sortValue, namespace, workload, container, clusterUUID}
	}
	return fmt.Sprintf(
		"((%s) > ? OR ((%s) IS NOT DISTINCT FROM ? AND %s > (?, ?, ?, ?)))",
		sortExpr, sortExpr, tie,
...
func nativeContainerSeekAfterLegacy(namespace, workload, container string) (string, []interface{}) {
	return "(rs.namespace, rs.workload, rs.container_name) > (?, ?, ?)", []interface{}{namespace, workload, container}
}
...
func applyNativeContainerPageSeek(query *gorm.DB, opts listoptions.ListOptions, sortExpr string) *gorm.DB {
	...
	if opts.AfterContainerSortPresent {
		clause, args := nativeContainerSeekAfter(
			...
			opts.AfterContainerClusterUUID, opts.AfterNamespace, opts.AfterWorkload, opts.AfterContainer,
		)
		return query.Where(clause, args...)
	}
	clause, args := nativeContainerSeekAfterLegacy(opts.AfterNamespace, opts.AfterWorkload, opts.AfterContainer)
	return query.Where(clause, args...)
}
